...
def train_neural_network(model, model_name, X_train, y_train, X_test, y_test, class_weights_dict):
    X_train_nn = X_train.values.reshape((X_train.shape[0], X_train.shape[1], 1))  # Reshape for Conv1D input
    X_test_nn = X_test.values.reshape((X_test.shape[0], X_test.shape[1], 1))      # Reshape for Conv1D input
...
nn_models = {
    ...
    "RNN": create_rnn_model(X_train.shape[1:])
}
